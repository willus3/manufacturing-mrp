...
test.describe.serial('6. Item-Supplier Links', () => {
  ...
  test('6.1 - Link supplier to item', async ({ page }) => {
    ...
    // Link them — 409 means link already exists from a prior run, which is fine
  ...
  test('6.3 - Multiple suppliers', async ({ page }) => {
    ...
    // Create a second supplier — may already exist from a prior run
    const suppSearch = await page.request.get(`${API}/suppliers?search=UAT+Second+Supplier`, { headers });
    const { data: existingSupps } = await suppSearch.json();
    const existingSupp = existingSupps.find((s) => s.code === 'UAT-SUP2');
    if (existingSupp) {
      secondSupplierId = existingSupp.id;
    } else {
    ...
    // Link second supplier (not preferred) — 409 means already linked, which is fine
    const linkRes = await page.request.post(`${API}/items/${rawItemId}/suppliers`, {
      headers,
